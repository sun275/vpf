...
def load_checkpoint_ema(config, model, optimizer, lr_scheduler, loss_scaler, logger, model_ema: ModelEma=None):
    ...
    if (
        config.EVAL_MODE
        and config.MODEL.TYPE == 'vpf'
        and hasattr(model, 'infer_init')
    ):
        model.infer_init()
    logger.info(msg)

    if model_ema is not None:
        ema_state_dict = checkpoint.get('model_ema', checkpoint['model'])
        if 'model_ema' not in checkpoint:
            logger.info(
                'Checkpoint has no model_ema entry; using model weights for '
                'the EMA evaluation copy.'
            )
        ema_msg = model_ema.ema.load_state_dict(ema_state_dict, strict=False)
        logger.info(ema_msg)
...
def load_checkpoint_val(config, model, logger):
    logger.info(f"==============> Resuming form {config.MODEL.RESUME}....................")
