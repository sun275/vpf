...
def parse_option():
    parser = argparse.ArgumentParser('VPF training and evaluation script', add_help=False)
    ...
    parser.add_argument("--local_rank", type=int, default=int(os.environ.get("LOCAL_RANK", -1)), help='local rank for DistributedDataParallel')
...
def main(config):
    ...
    logger.info(
        f"Created {type(model).__name__} with "
        f"{sum(parameter.numel() for parameter in model.parameters()):,} parameters"
    )
    
    if not (config.EVAL_MODE and config.MODEL.TYPE == 'vpf'):
        try:
    ...
    if config.MODEL.RESUME:
        ...
        if config.EVAL_MODE and config.MODEL.TYPE == 'vpf':
            try:
    ...
    for epoch in range(config.TRAIN.START_EPOCH, config.TRAIN.EPOCHS):
        ...
        # 前 100 个 Epoch (包含 20 个 Warmup) 保持固定
        # if epoch < 50:
        #     set_dno_status(model_without_ddp, frozen=True)
        # else:
        #     # 第 101 个 Epoch 开始解冻，进入 82% -> 85%+ 的微调演化阶段
        #     set_dno_status(model_without_ddp, frozen=False)
...
def set_dno_status(model, frozen=True):
    """
    根据开关状态固定或解冻全模型中所有 DNO2D 模块的参数
    """
    # 这里的 ".op." 对应你 DNOBlock 中的 self.op = DNO2D(...)
    ...
    # 仅在主进程打印，避免日志冗余
    if dist.get_rank() == 0:
        print(f" >>> [Strategy] DNO2D parameters are now {status}")
